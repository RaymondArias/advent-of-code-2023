        ...
        for i, line in enumerate(self.schematic):
            for j, value in enumerate(line):
                ...
                if not value.isdigit() and value == "*":
                    ...
                    # If there is exactly 2 add to gear ratio number
                    if len(adjacent_nums) == 2:
                        gear_number += adjacent_nums[0] * adjacent_nums[1]
...
def schematic_number() -> int:
    ...
    with open("day03b/data/input") as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()
            schematic.read_line(line)

    return schematic.get_gear_number()
